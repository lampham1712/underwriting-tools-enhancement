...
- **SystemStatus**: Represents the health state of an external integration.
    - `id`: Unique identifier. (UUID v7 nullable) PRIMARY KEY
    - `system_code`: Unique identifier (e.g., "CCR", "PAY_GATE"). (string max 20 chars).
    - `display_name`: Human-readable name. (string max 100 chars).
    - `status`: Enum (ONLINE, OFFLINE, UNKNOWN). (string nullable)
    - `last_checked`: Timestamp of the last ping. (ISO 8601 datetime nullable)
    - `next_check`: Timestamp of the next scheduled ping. (ISO 8601 datetime nullable)
    - `response_time_ms`: Duration of the ping response. (integer nullable)
    - `created_at`: Timestamp when the record was created. (ISO 8601 datetime nullable)
    - `updated_at`: Timestamp when the record was last updated. (ISO 8601 datetime nullable)
